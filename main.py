import languages
import pve_game
import pvp_game
import menu
import game_functions

# Settings defaults values
# could import statistic from existing file instead if we implement this feature
settings_values = {"board_size": 10, "number_of_ships": 3}
language_pack = languages.deutsch
player_1 = {"name": "Player 1", "score": 0}
player_2 = {"name": "Player 2", "score": 0}




# Start settings_menu
menu.clear_screen()
menu.show_main_menu()
print("\n")

# Locked in this loop until quit option is select and "break;" or "exit()" is reached
while True:
    try:
        main_menu_input = int(input(f'  {language_pack["input_field"]}: '))

        # Opening Game typ settings_menu and choosing between "PvP", "PvE", and "Go back"
        if main_menu_input == 1:
            menu.clear_screen()
            number_of_human_players = menu.game_typ()

            # Start game against bot
            if number_of_human_players == 1:
                pve_game.launch(player_1, player_2, settings_values)
                menu.show_main_menu()
                print("\n")

            # Start game against humain
            elif number_of_human_players == 2:
                pvp_game.launch(player_1, player_2, settings_values)
                menu.show_main_menu()
                print("\n")

            # Go back to main settings_menu
            elif number_of_human_players == 3:
                menu.clear_screen()
                menu.show_main_menu()
                print("\n")

        # Open settings_menu. If more option are added (number of ship, bot level, etc) It could open a sub settings_menu instead
        elif main_menu_input == 2:
            menu.clear_screen()
            settings_values = menu.settings_menu(settings_values)
            menu.clear_screen()
            menu.show_main_menu()
            print("\n")

        # Leave game
        elif main_menu_input == 3:
            menu.clear_screen()
            print(f" {language_pack['bye']}")
            exit()

        # Catch wrong input
        else:
            menu.show_main_menu()
            menu.invalid_input()

    # Catch wron value input
    except ValueError:
        menu.show_main_menu()
        menu.invalid_input()
        continue
